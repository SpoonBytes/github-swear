...
fs.readFile('swears.txt', (err, data) => {
	...
	console.log(`Beginning search for swear word "${swears[swearIndex]}."`);
	...
	const rateLimiter = setInterval(() => {
		...
		github.search.code({
			...
			if(swear.pageNumber === swear.pages) {
				console.log(`Ending search for swear word "${swears[swearIndex].word}."`);
				swearIndex += 1;
				console.log(`Starting next search for swear word "${swears[swearIndex].word}."`);
			} else {
