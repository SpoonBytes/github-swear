...
for (language of languages) {
	for (i = 0; i < swears.length; i = i + 3) {
		baseQueue.push(`${swears[i]} OR ${swears[i + 1]} OR ${swears[i + 2]} in:file language:"${language}"`);
	}
}

function memoryIsAvailable() {
	return os.freemem() <= os.totalmem() * .80;
}
...
// 			console.log(`Beginning search for ${query.language} files containing "${query.phrase}" on page 1.`);
