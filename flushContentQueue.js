...
let memoryAvailable = () => os.freemem() <= os.totalmem() * 0.8;
...
function readFile(file) {
	...
	github.repos
		...
		.then(result => {
			const content = new Buffer(result.data.content, "base64").toString(
				"utf8"
			);
		...
		.catch(err => {
			if (err.message.includes("rate limit exceeded")) {
				...
				console.log(
					`RATE LIMIT! RETRYING AFTER ${retryAfter}s - FILE: ${file.path}`
				);
			...
			if (err.message.includes("abuse detection mechanism")) {
				...
				console.log(
					`ABUSE DETECTION! RETRYING AFTER ${retryAfter}s - FILE: ${file.path}`
				);
				setTimeout(readFile, retryAfter * 1000, file);
			} else {
				console.log(err);
...
setInterval(() => {
	fs.writeFile(
		"contentQueue.json",
		JSON.stringify(contentQueue, null, 2),
		err => {}
	);
}, 60000);
