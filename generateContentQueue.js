...
for (language of languages) {
	for (i = 0; i < swears.length; i = i + 3) {
		baseQueue.push({
			phrase: `${swears[i]} OR ${swears[i + 1]} OR ${
				swears[i + 2]
			} in:file language:"${language}"`,
			language: language
		});
	}
}
	...
	github.search
		...
		.catch(err => {
			if (err.message.includes("rate limit exceeded")) {
				...
				console.log(
					`RATE LIMIT! RETRYING AFTER ${retryAfter}s - QUERY: ${
						query.phrase
					} PAGE: ${pageNumber}`
				);
			...
			if (err.message.includes("abuse detection mechanism")) {
				...
				console.log(
					`ABUSE DETECTION! RETRYING AFTER ${retryAfter}s - QUERY: ${
						query.phrase
					} PAGE: ${pageNumber}`
				);
				setTimeout(crawl, retryAfter * 1000, query, pageNumber);
